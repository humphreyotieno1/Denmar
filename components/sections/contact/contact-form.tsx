...
import { Send, CheckCircle, Plus, Minus, Users, Baby, Plane, ArrowRight, ArrowLeft } from "@/components/ui/huge-icons"
...
const contactSchema = z.object({
  ...
  return parseISO(data.travelDateTo) >= parseISO(data.travelDateFrom)
}, {
  message: "Travel end date must be on or after start date",
  path: ["travelDateTo"],
})
...
export function ContactForm({ countries = [] }: { countries?: any[] }) {
  ...
  const nextStep = async () => {
    const stepOneFields: Array<keyof ContactFormData> = [
      "firstName",
      "lastName",
      "email",
      "phone",
    ...
    const isOtherDestinationValid = destination === "other" ? await trigger("otherDestination") : true

    if (isStepOneValid && isOtherDestinationValid) {
      setStep(2)
    } else {
      toast.error("Please complete Step 1 before continuing.")
    }
  }
  ...
  return (
    <Card className="border-0 shadow-xl bg-white/95 backdrop-blur-sm">
      <CardHeader className="p-6 pb-2 tablet:p-8 tablet:pb-6">
        ...
        <p className="text-gray-600 text-base sm:text-lg">A quick 2-step form. Tell us about you first, then share your trip preferences.</p>
      </CardHeader>
      <CardContent className="p-5 tablet:p-8 pt-0">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <p className="font-semibold text-brand-primary">Your Details</p>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <p className="font-semibold text-brand-primary">Trip Preferences</p>
              </button>
            </div>
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in fade-in-50 slide-in-from-right-1 duration-300">
              {/* Name Fields */}
              ...
              <Button
                type="button"
                onClick={nextStep}
                className="w-full sm:w-auto bg-brand-accent rounded-full hover:bg-brand-accent/90 text-white"
              >
                Continue to Step 2
            ...
            <div className="space-y-6 animate-in fade-in-50 slide-in-from-left-1 duration-300">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Button
                  ...
                  className="w-full bg-brand-accent rounded-full hover:bg-brand-accent/90 text-white font-semibold h-12 text-base transition-all duration-200 hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
                    <div className="flex items-center gap-2">
