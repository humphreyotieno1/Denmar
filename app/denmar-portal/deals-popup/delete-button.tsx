...
export function DeletePopupButton({ id, title }: DeletePopupButtonProps) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await fetch(`/api/denmar-portal/deals-popup/${id}`, {
                method: "DELETE",
            })
            ...
            toast.success(`Popup "${title}" deleted`)
            router.refresh()
        } catch {
    ...
    return (
        <AlertDialog>
            ...
            <AlertDialogContent className="bg-white border-slate-200">
                <AlertDialogHeader>
                    <AlertDialogTitle className="text-slate-900">Delete Popup</AlertDialogTitle>
